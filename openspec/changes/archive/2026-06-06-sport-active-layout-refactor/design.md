...
所有状态（prep / countdown / active / paused / report）共享统一的三区结构，底栏位置始终固定在屏幕底部，状态切换时不产生视觉跳跃。

```
┌──────────────────┐
│     顶栏区域      │  ← 所有状态共享：运动名 + 关闭/返回
│──────────────────│
│                  │
│   主交互区       │  ← 根据状态变化内容，但不改变位置
│   (flex: 1)      │
│                  │
│──────────────────│
│     底栏区域      │  ← 所有状态共享：操作按钮固定在此
└──────────────────┘
```

**当前问题**：暂停页使用 `justifyContent: 'center'` 将按钮推到屏幕中央，与 active 页底栏位置不一致，状态切换时有跳跃感。

**重构规则**：
- 底栏区域使用绝对定位或固定 flex 底部，不随主内容居中
- 主交互区使用 `flex: 1` 自然填充，内容用 `alignItems: 'center'` 水平居中，但垂直方向不强制居中（用 `paddingTop` 或 `marginTop` 控制位置）
- 各状态的底栏按钮样式/位置保持一致，仅按钮语义随布局类型变化
