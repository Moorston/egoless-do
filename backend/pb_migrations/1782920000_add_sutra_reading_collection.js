...
// Creates sutra_reading_sessions collection for sync.
...
migrate((txApp) => {
  ...
  ensureCollection("sutra_reading_sessions", {
    name: "sutra_reading_sessions", type: "base", system: false,
    fields: syncFields("reading_id"),
    indexes: syncIndexes("sutra_reading_sessions", "reading_id"),
    options: {},
  });
  ...
  setRules("sutra_reading_sessions", AUTH_RULES);
}, (txApp) => {
  try { var c = txApp.findCollectionByNameOrId("sutra_reading_sessions"); if (c) txApp.deleteCollection(c); } catch {}
});
