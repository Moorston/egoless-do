    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 300, background: TH.bg, overflowY: 'auto' }}>
      <div style={{ maxWidth: 390, margin: '0 auto' }}>
        ...
        <div style={{ padding: '0 16px' }}>
          {medHistory.length === 0 && <div style={{ textAlign: 'center', color: TH.sub, padding: '40px 0', fontSize: 13 }}>{T('noHistory')}</div>}
          {medHistory.map((m, i) => (
            <div key={i} style={{ background: TH.card, borderRadius: 16, padding: 14, marginBottom: 10, border: `1px solid ${TH.border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <div style={{ fontWeight: 600, fontSize: 15, color: TH.text }}>{m.date}</div>
                <div style={{ fontSize: 13, color: TH.sub, marginTop: 2 }}>{m.mood}</div>
              </div>
              <div style={{ fontWeight: 700, color: P, fontSize: 15 }}>{m.dur}</div>
            </div>
          ))}
        </div>
      </div>
