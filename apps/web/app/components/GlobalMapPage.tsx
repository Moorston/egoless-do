...
import { THEMES, GLOBAL_USERS } from '@egoless/core';
import type { GlobalUser } from '@egoless/core';
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 300, background: '#000', display: 'flex', flexDirection: 'column' }}>
        ...
        {GLOBAL_USERS.map((u) => (
          <button key={u.id} onClick={() => setSel(u)}
            style={{ position: 'absolute', left: `${((u.lng + 180) / 360) * 100}%`, top: `${((90 - u.lat) / 180) * 100}%`,
              ...
              fontSize: 13, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 700, zIndex: 2 }}>
            {u.name[0]}
          </button>
        ))}
        {sel && (
          <div style={{ position: 'absolute', bottom: 80, left: 16, right: 16, background: 'rgba(0,0,0,.85)', borderRadius: 12, padding: 12, zIndex: 3 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div>
                <div style={{ fontWeight: 700, fontSize: 14, color: '#fff' }}>{sel.name}</div>
                <div style={{ fontSize: 12, color: 'rgba(255,255,255,.6)', marginTop: 3 }}>打卡 {sel.days} 天 · {sel.sport}</div>
                <div style={{ fontSize: 13, color: 'rgba(255,255,255,.5)', marginTop: 2 }}>{sel.since} 开始 · 坚持了 {sel.duration}</div>
              </div>
      ...
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 12, background: 'linear-gradient(180deg,rgba(0,0,0,.6),transparent)', zIndex: 10 }}>
        ...
        <span style={{ fontWeight: 700, fontSize: 16, color: '#fff' }}>全球脉动</span>
      </div>
      ...
      <div style={{ position: 'absolute', bottom: 24, left: '50%', transform: 'translateX(-50%)', zIndex: 10 }}>
        <button onClick={() => setShowBoard(true)}
          style={{ padding: '12px 28px', borderRadius: 24, border: 'none', background: `${P}E0`, color: '#fff', fontWeight: 700, fontSize: 15, cursor: 'pointer', boxShadow: `0 4px 20px ${P}60`, backdropFilter: 'blur(8px)' }}>
          🏆 打卡排行榜
        </button>
      </div>
    </div>
        ...
        {sorted.map((u, i) => (
          <div key={u.id} style={{ background: TH.card, borderRadius: 16, marginBottom: 10, border: `1px solid ${TH.border}`, display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px' }}>
            <div style={{ width: 30, height: 30, borderRadius: 15,
              ...
              display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 800, fontSize: 13, flexShrink: 0 }}>{i + 1}</div>
            <div style={{ flex: 1 }}>
              <div style={{ fontWeight: 600, fontSize: 14, color: TH.text }}>{u.name}</div>
            ...
            <div style={{ textAlign: 'right' }}>
              ...
              <div style={{ fontSize: 12, color: TH.sub }}>{tab === 0 ? '天(当前)' : '天(总计)'}</div>
            </div>
          </div>
        ))}
      </div>
