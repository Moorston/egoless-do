...
import { THEMES, t, COLORS, FONT_BODY, FONT_SUB, FONT_STAT_SECTION } from '@egoless-do/core';
  ...
  return (
    <div style={{ padding: '20px 16px 0', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexShrink: 0, position: 'relative', zIndex: 1 }}>
      ...
      <div style={{ textAlign: 'right' }}>
        ...
        <div style={{ fontSize: FONT_STAT_SECTION, fontWeight: 800, color: COLORS.ORANGE, lineHeight: 1.2 }}>{streak} <span style={{ fontSize: FONT_BODY }}>{T('days')} <Flame size={20} style={{verticalAlign:'middle', color: COLORS.ORANGE}} /></span></div>
      </div>
    </div>
  );
}
